...
namespace CrateExpectations.Combat
{
    ...
    public sealed class WeaponSocket : MonoBehaviour
    {
        ...
        /// <summary>
        /// Отпускает оружие в мир и перестаёт им распоряжаться. Возвращает отпущенный
        /// объект (или <c>null</c>, если рука была пуста) - что с ним делать дальше,
        /// решает вызывающий: сокет умеет держать и отпускать, но не умеет ронять.
        /// <para>
        /// Метод обязан быть здесь, а не «отцепить руками снаружи»: посадка применяется
        /// в <see cref="LateUpdate"/> КАЖДЫЙ кадр, и клинок, просто вынутый из иерархии,
        /// следующим же кадром прыгнул бы обратно в ладонь. Отпускание - это в первую
        /// очередь «сокет забыл про оружие», а уже потом смена родителя
        /// </para>
        /// </summary>
        public GameObject Release()
        {
            GameObject released = _instance;

            if (released == null)
                return null;

            // worldPositionStays: клинок остаётся ровно там, где его застала смерть,
            // а не схлопывается в начало координат сцены
