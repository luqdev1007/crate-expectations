...
namespace CrateExpectations.Cargo.Catalog
{
    ...
    ///
    /// <para>Что через Addressables идёт, а что нет. Через бандлы едет только то, что
    /// рисуется, т.е префабы ящиков. Типы груза приходят из
    ...
    /// Unity кладёт копию в бандл и эта копия - другой объект, не оригинал в сборке.
    ...
    public sealed class AddressableCargoCatalog : ICargoCatalog, IDisposable
    {
            ...
            string cargoTypeKey, Vector3 position, Quaternion rotation, CancellationToken ct = default)
        {
            ...
            if (instance.TryGetComponent(out CargoBox box) == false)
            {
