...
namespace CrateExpectations.Player.Combat
{
    ...
    /// как и был. С УБРАННОЙ саблей заряд не копится ни у одного направления: бить из
    /// этого состояния всё равно нельзя, а занятые руки от висящего заряда - можно.
    /// </para>
    /// <para>
    ...
    public sealed class PlayerWeaponController : MonoBehaviour, IWeaponStateSource
    {
        ...
        private void OnAttackPressed()
        {
            ...
            // С убранной саблей заряд не начинаем вообще. Ударить из Sheathed нельзя -
            // машина состояний пускает в удар только из Ready, - и накопленное удержание
            // всё равно истекло бы в буфере впустую. Хуже другое: копящийся заряд
            // поднимает IsBusy, то есть занятость рук уезжает в Combat, и игрок, зажавший
            // ЛКМ без оружия, теряет возможность взять груз и поднять листок.
            // Раньше это задевало одно направление вперёд, теперь заряжены почти все,
            // и незажатая клавиша молча блокировала бы половину взаимодействий.
            //
            // Условие именно на Sheathed, а не на CanAttack: то ложно ещё и посреди удара,
            // а там заряд обязан копиться - на этом стоит буферизация следующего приёма
            if (charge > 0f && _machine.State != WeaponState.Sheathed)
            {
